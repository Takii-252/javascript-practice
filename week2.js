...
// そのため、「Dogクラス」内にはまだ何もメソッドは
// 定義されていませんが、「Animalクラス」に定義されている
...
//*学習41 オーバライド2
//コンストラクタもオーバーライドすることができます。例えば、子クラスにプロパティを追加したい場合などに用います。
...
// ※コンストラクタは初期設定であり、初期設定（プロパティ）に何か情報を追加したい場合に追加するイメージ、またコンストラクタはnewで必ず最初に呼び出される
// ※メソッドは追加情報や振る舞いでコンストラクタと同じように引数を渡すことが出来る
class Bard extends Animal8 {
  constructor(name, age, breed) {
    super(name, age);
    // 元のコンストラクタをsuperを使ってオーバライドする
    this.breed = breed;
    // 新しい引数を追加し、コンストラクタにも追加する
  }
  ...
  info() {
    ...
    // 元のinfoメソッドをオーバライドする
    const humanage = this.gethumanage();
    console.log(`鳥年齢で${humanage}歳です`);
    console.log(`鳥の種類は${this.breed}です`);
    // メソッドの外にあるのでthisを付けないといけない。thisは、一言で言うと、
    // 「メソッドの内側から、そのメソッドの外側にある、インスタンス自身の『持ち物（プロパティ）』や『能力（他のメソッド）』にアクセスするための、唯一の合言葉」
  }
  gethumanage() {
    return this.age * 5;
  }
